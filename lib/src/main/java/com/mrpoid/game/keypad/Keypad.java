...
public class Keypad extends Director implements ClickCallback {
	 ...
	 * @param c
	 */
	public Keypad(Context c, boolean edit) {
		init(c);
		this.mEditMode = edit;
	}
...
//		setLayouter(new DefKeyLayouter());
		...
		DP1 = c.getResources().getDimensionPixelSize(R.dimen.dp1);
		DP5 = c.getResources().getDimensionPixelSize(R.dimen.dp5);
	}
...
//					float scale = Float.valueOf(pullParser.getAttributeValue(1));
	...
	public void reset() {
		...
		floatMenuBtn.setSize(50*DP1, 40*DP1);
		floatMenuBtn.setPosition(DP5, DP5);
		addChild(floatMenuBtn);
		
		floatMenuBtn.setVisible(true);
